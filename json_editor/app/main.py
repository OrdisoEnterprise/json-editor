...
import os
import json
from json_editor.app.json_editor import *
import importlib.util
import glob

# from json_editor.app.plugin_ import Generate
...
class JsonEditor(QMainWindow):
    def __init__(self, schema, input):
        self.modules = {}
        self.docks = {}
        self.actions = {}
    ...
    def loadPlugins(self):
        tMenu = self.menuBar().addMenu(self.tr('Plugin'))
        plgdir = os.path.abspath("C:/TTTech/asanchez_personal/env_manager/json-editor/tests/data/plugins/")
        for plg in glob.glob('%s/*.py' % (plgdir)):
            plgName = os.path.basename(plg)[:-3]
            spec = importlib.util.spec_from_file_location(plgName, plg)
            plgM = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(plgM)
            self.plugin_manager.register_plugin(plgM.ConfigModelSamplePlugin().get_id(), plgM.ConfigModelSamplePlugin())
            sItem = plgM.Plugin(self)
            tMenu.addAction(sItem)

            # self.plugin_manager.execute_plugin("sample_plugin", action="open", arg1="component_value1", arg2="component_value2")
            # validation_result = self.plugin_manager.execute_plugin("sample_plugin", action="validate", arg1="component_value1", arg2="component_value2")
            # print("Validation Result:", validation_result)
            # generated_content = self.plugin_manager.execute_plugin("sample_plugin", action="generate", arg1="component_value1", arg2="component_value2")
    ...
    def creMenu(self):
        ...
        tMenu.addAction(sItem)
        # Validate Ctrl+E
        sItem = QAction(self.tr('Validate'), self)
        sItem.setShortcut('Ctrl+E')
        sItem.setStatusTip('Validate the JSON configure file.')
        sItem.triggered.connect(self.mVal)
        tMenu.addAction(sItem)
        # Generate Ctrl+G
    ...
    def mVal(self):
        ...
        gdir = os.path.abspath(gdir) + '/validation'
        os.makedirs(gdir, exist_ok=True)
        QMessageBox(QMessageBox.Information, 'Info',
                    'Validation of configuration ends successfully !\n<%s>\n' % (gdir)).exec_()
    ...
    def mGen(self):
        ...
        gdir = os.path.dirname(self.jsonFile)
        gdir = os.path.abspath(gdir) + '/generated'
        os.makedirs(gdir, exist_ok=True)
        cfgs = []
        for title, module in self.modules.items():
            cfg = module.toJSON()
            ...
            if cfg.endswith('.json'):
                with open(cfg) as f:
                    ...
                    if 'class' in con:
                        ...
                        if cls in self.plugin_manager.plugins:
                            self.plugin_manager.execute_plugin(cls, action="generate", config=cfgs)
                        else:
...
def run_json_editor(schema_file: str, input_file= str):
    ...
    qtGui = JsonEditor(schema=schema_file, input=input_file)
    qtGui.show()
    qtApp.exec_()
...
def main():
    ...
    run_json_editor(schema_file=args.schema, input_file=args.input)
