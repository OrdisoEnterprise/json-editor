...
class JsonBase(QTreeWidgetItem):
    ...
    def onItemSelectionChanged(self):
        ...
        if self.isList():
            ...
            schema = resolve_if_ref(dict(items))
            if self.listMaxAllowed() > self.childCount():
                if self.is_enabled(self.title):
                    self.root.actions[Index].setText('Add %s' % (schema['title']))
...
class JsonArray(JsonBase):
    ...
    def reload(self):
        if '__init__' in self.schema:
            items = dict(self.schema['items'])
            items = resolve_if_ref(dict(items))
        ...
        # items = dict(self.schema['items'])
        # schema = resolve_if_ref(dict(items))
        # self.addChild(JsonItem(schema['title'], schema, self.root, self))
    ...
    def onAction_Add(self, what):
        items = self.schema['items']
        schema = resolve_if_ref(dict(items))
        if schema['title'] == what:
            if self.isList():
                if self.listMaxAllowed() > self.childCount():
                    self.addChildObj(JsonItem(schema['title'] , schema, self.root, self))
                else:
                    logging.error('Error:Maximum %s for %s is %s!' % (what, schema['title'] , mx))
            self.setExpanded(True)
        else:
            logging.error('add with what=%s != %s' % (what, schema['title'] ))
...
def resolve_if_ref(schema):
    # Check if the schema has a "$ref" key
    if "$ref" in schema:
        # Extract the reference and resolve it using the provided resolver
        ref = schema["$ref"]
        global resolver
        url, remote_schema = resolver.resolve(ref)
        return remote_schema
    else:
        # If no "$ref" key is present, return the original schema
        return schema
